image:
	sudo $(HOME)/bin/singularity create --size 3000 singularity-fenics-dev-env.img
	sudo $(HOME)/bin/singularity bootstrap singularity-fenics-dev-env.img Singularity
	sudo $(HOME)/bin/singularity copy singularity-fenics-dev-env.img fenics-build /opt/bin/fenics-build
